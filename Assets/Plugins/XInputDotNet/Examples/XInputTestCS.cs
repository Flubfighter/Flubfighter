...
public class XInputTestCS : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.localRotation *= Quaternion.Euler(0.0f, state.ThumbSticks.Left.X * 25.0f * Time.unscaledDeltaTime, 0.0f);
    }

    void OnGUI()
    {
        string text = "Use left stick to turn the cube, hold A to change color\n";
		text += string.Format("IsConnected {2} {0} Packet #{1}\n", state.IsConnected, state.PacketNumber, playerIndex);
